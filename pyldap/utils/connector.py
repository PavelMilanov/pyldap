...
class Ldap3Connector:
    ...
    async def search_organizations(self) -> Dict | None:
        try:
            with Connection(self._SERVER, user=self._LOGIN, password=self._PASSWORD, authentication=NTLM) as dc:
                dc.search(search_base=f'ou=ARMs,ou={self._OU},dc={self._DC1},dc={self._DC2}', search_scope=LEVEL, search_filter='(objectCategory=organizationalUnit)', attributes=ALL_ATTRIBUTES)
